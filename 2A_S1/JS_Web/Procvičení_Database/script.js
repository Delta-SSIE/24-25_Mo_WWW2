const json = `[{"id":1,"first_name":"Westbrooke","last_name":"Macilhench","email":"wmacilhench0@dedecms.com","ip_address":"5.70.171.48"},
    ...
    let nacistbtn = document.getElementById('nacist');
    let input = document.getElementById('id');
    let txt = document.getElementById('user');
    ...
    input.addEventListener("keyup",nacistuzivatele);

    nacistbtn.addEventListener("click",nacistdata);
    ...
    function edituser()
    {
        ...
        window.localStorage.setItem(editid,JSON.stringify(editedUser));
        for (let index = 0; index < editinputs.length; index++) {
            editinputs[index].value = null;
        }
        editid = null;
        nacistuzivatele();
    }

    function puteditinfo(user)
    {
        for (let index = 0; index < editinputs.length; index++) {
    ...
    function nacistuzivatele()
    {
        ...
        {
            let uzivatel = JSON.parse(window.localStorage.getItem(input.value));
            txt.textContent = `${uzivatel.id}: ${uzivatel.first_name} ${uzivatel.last_name}`;
            txt.addEventListener("click",() => puteditinfo(uzivatel));
        } else {
    ...
    function nacistdata()
    {
        ...
        for(let uzivatel of pole_objektu)
        {
            window.localStorage.setItem(uzivatel.id,JSON.stringify(uzivatel));
        }
    }