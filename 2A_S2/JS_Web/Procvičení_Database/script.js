const json = `[{"id":1,"first_name":"Westbrooke","last_name":"Macilhench","email":"wmacilhench0@dedecms.com","ip_address":"5.70.171.48"},
    ...
    {
        if(window.localStorage.getItem(input.value) != null)
        {
            useroutput.innerHTML = "";
            let user = window.localStorage.getItem(input.value);
            user = JSON.parse(user);
            for(let zaznam of Object.entries(user))
            {
    ...
    function Nactidata()
    {
        ...
        console.log(pole_uzivatelu);
        window.localStorage.clear();
        for (let index = 0; index < pole_uzivatelu.length; index++) {
            window.localStorage.setItem(pole_uzivatelu[index].id,JSON.stringify(pole_uzivatelu[index]));
        }
    }