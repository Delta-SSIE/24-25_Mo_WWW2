const films = new Map([
  ...
  let txt = document.getElementById('details');

  let search = document.getElementById('search');
  search.addEventListener("input",Hledej)

  function Hledej() {
    seznam.innerHTML = "";
    for(let nazevfilmu of films.keys())
      {
        if(nazevfilmu.includes(search.value))
        {
          let item = document.createElement('li');
          item.innerHTML = nazevfilmu;
          item.addEventListener("click", () => details(nazevfilmu));
  ...
  {
    console.log(film);
    let detail = films.get(film);
    txt.innerHTML = `Rok vydání ${detail.year}; Žánr ${detail.genre}; Režie: ${detail.director}`;
  }
