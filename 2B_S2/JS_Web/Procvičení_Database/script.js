const json = `[{"id":1,"first_name":"Westbrooke","last_name":"Macilhench","email":"wmacilhench0@dedecms.com","ip_address":"5.70.171.48"},
 ...
 let nactibtn = document.getElementById('nacist');
 ...
 nactibtn.addEventListener("click",nactidata);

 function nactidata()
 {
    window.localStorage.clear();
    let pole_uzivatelu = JSON.parse(json);
    for (let uzivatel of pole_uzivatelu) {
 ...
 function getuser()
 {
    ...
    {
        let finduser = JSON.parse(window.localStorage.getItem(input.value));
        text.textContent = `${finduser.id}: ${finduser.first_name} ${finduser.last_name}`;
        text.addEventListener("click",() => putedit(finduser));
    } else {
        text.textContent = "";
    }
 ...
 {
    let pole_hodnot = Object.values(user)
   for (let index = 0; index < editinputs.length; index++) {
        editinputs[index].value = pole_hodnot[index+1];
   }
   editid = pole_hodnot[0];
 }
 ...
 function edituser()
 {
    ...
    window.localStorage.setItem(editid,JSON.stringify(euser));
    editid = null;
    for (let index = 0; index < editinputs.length; index++) {
